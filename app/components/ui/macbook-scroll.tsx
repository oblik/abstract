...
export const MacbookScroll = ({ src, showGradient, title, badge }: MacbookScrollProps) => {
  ...
  const scaleX = useTransform(scrollYProgress, [0, 0.1, 1], [
    1.2,
    isMobile ? 1 : 1.5,
    isMobile ? 1 : 1.5,
  ]);
  const scaleY = useTransform(scrollYProgress, [0, 0.1, 1], [
    0.6,
    isMobile ? 1 : 1.5,
    isMobile ? 1 : 1.5,
  ]);
  ...
  return (
    <div
      ...
      "
    >
      {/* Hidden placeholder so the component doesn't collapse */}
      <motion.h2 style={{ opacity: 0 }} className="text-transparent">
        Placeholder
      </motion.h2>
...
export const OptionKey = ({ className }: OptionKeyProps) => {
  return (
    <svg
      fill="none"
      version="1.1"
