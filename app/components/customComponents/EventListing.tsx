      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[10px] w-full">
        {events &&
          ...
          events.map((event) => (
            <div
              key={event._id}
              className={`event-card w-full rounded-[11.5px] lg:rounded-lg transform transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg cursor-pointer ${event.status + "_event"
                }`}
            >
              {event.marketId?.length < 2 ? (
                ...
                >
                  <EventCard
                    ...
                    totalPool={`$${event.marketId?.[0]?.volume
                    ...
                    yesButtonLabel={`Buy ${firstLetterCase(event.marketId[0]?.outcome?.[0]?.title) || "Yes"
                      }`}
                    noButtonLabel={`Buy ${firstLetterCase(event.marketId[0]?.outcome?.[1]?.title) || "No"
                      }`}
                    yesPotential={
                      (event.marketId[0]?.outcomePrices &&
