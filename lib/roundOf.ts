...
declare global {
  interface Number {
    ...
    toFixedNoRoundingNoDecimals(n: number): string;
  }
}

Number.prototype.toFixedNoRounding = function (n: number): string {
	if (n > 100) {
...
export const toFixed = (item: any, type: number = 2): string => {
	try {
...
export const priceFixed = (item: any): string | undefined => {
	try {
		if (!isEmpty(item) && !isNaN(item)) {
...
export const currencyFormat = (item: any): string => {
	try {
		if (!isEmpty(item) && !isNaN(item)) {
			item = item.toString();
...
export const randomstring = (x: any): string => {
        try {
                var result = "";
...
export const longNumbers = (x: any, n: number): string => {
	try {
		x = parseFloat(x);
		if (!isNaN(x)) {
			if (x < 0) {
...
export const approxDecimal = (x: number): string => {
	try {
...
export const avoidExponents = (x: number): number => {
	try {
		if (x < 0.000001) {
			return 0;
		}
...
export const longNumbersNoDecimals = (x: any, n: number): string => {
	try {
		if (typeof x === "string") {
			x = parseFloat(x);
		}
...
export const shortenNumber = (input: any): string | number => {
	let num: number;
...
export const marketCapShorten = (input: any): string | number => {
	let num: number;
	if (input !== 0 && input < 1000) {
		return input;
	}
...
export const formatDecimals = (number: any): string | null | undefined => {
	if (number === null || number === undefined) return number;

	let formattedNumber: any = number;
	...
	return `${integerPart}.${firstDecimal}${zeroCount > 0 ? zeroCount.toString() : ""}${significantDigits || ""}`;
};


export const eToDecimal = (n: any): string => {
	try {
		const sign = +n < 0 ? "-" : "",
			toStr = n.toString();
		if (!/e/i.test(toStr)) {
...
export const trunc = (x: any, n: number): string => {
	try {
