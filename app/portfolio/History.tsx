...
const History: React.FC<HistoryProps> = (props) => {
  ...
  const [ClosedPnL, setClosedPnL] = useState<GroupedByEvent>({});
  const [tradeHistory, setTradeHistory] = useState<any[]>([])
  const [tradeOpen, setTradeOpen] = useState<boolean>(false)
  const [selectedMarketOutcome, setSelectedMarketOutcome] = useState<any[]>([])
  ...
  const formatClosedPnL = useCallback((data: any[]) => {
    const groupedByEvent: GroupedByEvent = {};
  ...
  const handleTradeOpen = async (id: string, outcomes: any[]) => {
                        ...
                        {marketIds.map((marketId, idx) => {
                          ...
                          return (
                            <tr key={marketId}>
                              ...
                              <td className={`${(m.shares || 0) > 0 ? "" : "text-gray-500"}`}>{(m.shares || 0) > 0 ? `$${Number(m.shares || 0).toFixed(2)}` : "--"}</td>
              ...
              <tbody>
                {tradeHistory?.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>${toFixedDown(item?.fee ? item.fee / 100 : 0, 5)}</td>
