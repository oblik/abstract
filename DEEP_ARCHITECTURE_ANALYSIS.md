...
const { isConnected, address } = useSelector(state => state?.walletconnect?.walletconnect); // ❌ Should use WalletContext instead!
...
const config = {
  ...
  getLoginInfo: process.env.NODE_ENV === "production" 
...
const { isConnected, address } = useSelector(state => 
  state?.walletconnect?.walletconnect);  // ❌ External from Redux (wrong!)
```
...
const { balance, inOrder, position } = useSelector(state => state.wallet.data);
```
